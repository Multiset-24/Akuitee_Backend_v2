...
const approveStagedContent = AsyncHandler(async (req, res) => {
  ...
  if (stagedContent.Action === "Update") {
    if (Action === "Approved") {
      const content = await Content.findById(stagedContent.OriginalContentId);
      if (!content) {
        throw new ApiError(404, "Content not found");
      }
      content.set(stagedContent); //set the content to the staged content
      await content.save();
      ...
      res
        .status(200)
        .json(new ApiResponse(200, "Content update rejected successfully"));
    }
    const user = await User.findById(stagedContent.Author);
    if (!user) {
      throw new ApiError(404, "User not found");
    }
    const message = `Your content has been ${Action} for the ${stagedContent.Action} request , contact the super admin for more information`;
  ...
  res
    .status(201)
    .json(
      new ApiResponse(201, "Content approved and saved successfully", content)
    );
});
